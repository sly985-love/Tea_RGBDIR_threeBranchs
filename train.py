...
from utils.datasets import create_dataloader, create_dataloader_rgb_ir,create_dataloader_rgb_ir_dep # create_dual_dataloader,create_dataloader_rgb_ir_dep
...
def train_rgb_ir_depth(hyp, opt, device, tb_writer=None):
    logger.info(colorstr('hyperparameters: ') + ', '.join(f'{k}={v}' for k, v in hyp.items()))
    save_dir, epochs, batch_size, total_batch_size, weights, rank = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank
    ...
    train_path_ir = data_dict['train_ir']
    test_path_ir = data_dict['val_ir']
    train_path_dep = data_dict['train_dep']
    test_path_dep = data_dict['val_dep']
    ...
    # Trainloader
    dataloader, dataset = create_dataloader_rgb_ir_dep(train_path_rgb, train_path_ir,train_path_dep, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    mlc = np.concatenate(dataset.labels, 0)[:, 0].max()  # max label class
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader_rgb_ir_dep(test_path_rgb, test_path_ir,test_path_dep,imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '))
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if flage_visual:
                ...
                for num in range(batch_size):
                    ...
                    image = image.squeeze(0)  # remove the fake batch dimension
                    image = unloader(image)
                    image.save('example_%s_%s_%s_dep.jpg'%(str(epoch), str(i), str(num)))
            ...
            with amp.autocast(enabled=cuda):
                # pred = model(imgs)  # forward
                pred = model(imgs_rgb, imgs_ir, imgs_dep)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                if rank != -1:
                    loss *= opt.world_size  # gradient averaged between devices in DDP mode
                if opt.quad:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5s_fusion_transformerx3_apples.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/multispectral/apple.yaml', help='data.yaml path')
    ...
    parser.add_argument('--project', default=r'D:\帅璐宇\RGBD_TEA\multispectral_3', help='save to project/name')
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            ...
            train_rgb_ir_depth(hyp, opt, device, tb_writer)

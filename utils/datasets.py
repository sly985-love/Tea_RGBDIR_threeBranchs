...
def create_dataloader_rgb_ir_dep(path1, path2, path3,  imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', sampler=None):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    FQY  载入多模态数据 （RGB 和 IR）
    """
    def __init__(self, path_rgb, path_ir, path_dep, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
        self.hyp = hyp
        ...
        try:
            ...
            # -----------------------------  dep   -----------------------------
            for p_dep in path_dep if isinstance(path_dep, list) else [path_dep]:
                p_dep = Path(p_dep)  # os-agnostic
                if p_dep.is_dir():  # dir
                    f_dep += glob.glob(str(p_dep / '**' / '*.*'), recursive=True)
                    # f = list(p.rglob('**/*.*'))  # pathlib
                elif p_dep.is_file():  # file
                    with open(p_dep, 'r') as t:
                        t = t.read().strip().splitlines()
                        parent = str(p_dep.parent) + os.sep
                        f_dep += [x.replace('./', parent) if x.startswith('./') else x for x in
                                  t]  # local to global path
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # local to global path (pathlib)
                else:
                    raise Exception(f'{prefix}{p_dep} does not exist')

            self.img_files_rgb = sorted([x.replace('/', os.sep) for x in f_rgb if x.split('.')[-1].lower() in img_formats])
            self.img_files_ir = sorted([x.replace('/', os.sep) for x in f_ir if x.split('.')[-1].lower() in img_formats])
            self.img_files_dep = sorted([x.replace('/', os.sep) for x in f_dep if x.split('.')[-1].lower() in img_formats])

            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
            assert (self.img_files_rgb, self.img_files_ir, self.img_files_dep), (f'{prefix}No images found', f'{prefix}No images found', f'{prefix}No images found')
        except Exception as e:
            raise Exception(f'{prefix}Error loading data from {path_rgb,path_ir,path_dep}: {e}\nSee {help_url}')
        ...
        # Check dep cache
        self.label_files_dep = img2label_paths(self.img_files_dep)  # labels
        # print(self.label_files)
        cache_dep_path = (p_dep if p_dep.is_file() else Path(self.label_files_dep[0]).parent).with_suffix(
            '.cache')  # cached labels
        if cache_dep_path.is_file():
            cache_dep, exists_dep = torch.load(cache_dep_path), True  # load
            if cache_dep['hash'] != get_hash(
                    self.label_files_dep + self.img_files_dep) or 'version' not in cache_dep:  # changed
                cache_dep, exists_dep = self.cache_labels(self.img_files_dep, self.label_files_dep,
                                                        cache_dep_path, prefix), False  # re-cache
        else:
            cache_dep, exists_dep = self.cache_labels(self.img_files_dep, self.label_files_dep,
                                                    cache_dep_path, prefix), False  # cache

        # Display cache
        nf_rgb, nm_rgb, ne_rgb, nc_rgb, n_rgb = cache_rgb.pop('results')  # found, missing, empty, corrupted, total
        nf_ir, nm_ir, ne_ir, nc_ir, n_ir = cache_ir.pop('results')  # found, missing, empty, corrupted, total
        nf_dep, nm_dep, ne_dep, nc_dep, n_dep = cache_dep.pop('results')  # found, missing, empty, corrupted, total
        if exists_rgb:
            d = f"Scanning RGB '{cache_rgb_path}' images and labels... {nf_rgb} found, {nm_rgb} missing, {ne_rgb} empty, {nc_rgb} corrupted"
            tqdm(None, desc=prefix + d, total=n_rgb, initial=n_rgb)  # display cache results
        if exists_ir:
            ...
            tqdm(None, desc=prefix + d, total=n_ir, initial=n_ir)  # display cache results
        if exists_dep:
            d = f"Scanning DEP '{cache_rgb_path}' images and labels... {nf_dep} found, {nm_dep} missing, {ne_dep} empty, {nc_dep} corrupted"
            tqdm(None, desc=prefix + d, total=n_dep, initial=n_dep)  # display cache results
        ...
        # Read DEP cache
        cache_dep.pop('hash')  # remove hash
        cache_dep.pop('version')  # remove version
        labels_dep, shapes_dep, self.segments_dep = zip(*cache_dep.values())
        self.labels_dep = list(labels_dep)
        self.shapes_dep = np.array(shapes_dep, dtype=np.float64)
        self.img_files_dep = list(cache_dep.keys())  # update
        self.label_files_dep = img2label_paths(cache_dep.keys())  # update
        if single_cls:
            for x in self.labels_dep:
                x[:, 0] = 0

        n_dep = len(shapes_dep)  # number of images
        bi_dep = np.floor(np.arange(n_dep) / batch_size).astype(np.int)  # batch index
        nb_dep = bi_dep[-1] + 1  # number of batches
        self.batch_dep = bi_dep  # batch index of image
        self.n_dep = n_dep
        self.indices_dep = range(n_dep)
        ...
        if self.rect:
            ...
            # DEP
            # Sort by aspect ratio
            s_dep = self.shapes_dep  # wh
            ar_dep = s_dep[:, 1] / s_dep[:, 0]  # aspect ratio
            irect_dep = ar_dep.argsort()
            self.img_files_dep = [self.img_files_dep[i] for i in irect_dep]
            self.label_files_dep = [self.label_files_dep[i] for i in irect_dep]
            self.labels_dep = [self.labels_dep[i] for i in irect_dep]
            self.shapes_dep = s_dep[irect_dep]  # wh
            ar_dep = ar_dep[irect_dep]

            # Set training image shapes
            shapes_dep = [[1, 1]] * nb_dep
            for i in range(nb_dep):
                ari_dep = ar_dep[bi_dep == i]
                mini, maxi = ari_dep.min(), ari_dep.max()
                if maxi < 1:
                    shapes_dep[i] = [maxi, 1]
                elif mini > 1:
                    shapes_dep[i] = [1, 1 / mini]

            self.batch_shapes_dep = np.ceil(np.array(shapes_dep) * img_size / stride + pad).astype(np.int) * stride

        # Cache images into memory for faster training (WARNING: large datasets may exceed system RAM)
        self.imgs_rgb = [None] * n_rgb
        self.imgs_ir = [None] * n_ir
        self.imgs_dep = [None] * n_dep
    ...
    def __getitem__(self, index):
        ...
        index_rgb = self.indices_rgb[index]  # linear, shuffled, or image_weights
        index_ir = self.indices_ir[index]  # linear, shuffled, or image_weights
        index_dep = self.indices_dep[index]  # linear, shuffled, or image_weights
        ...
        if mosaic:
            ...
            # img, labels = load_mosaic(self, index)
            img_rgb, labels_rgb, img_ir, labels_ir, img_dep, labels_dep = load_mosaic_RGB_IR_DEP(self, index_rgb, index_ir, index_dep)
            ...
            # Load image
            img_rgb, img_ir, img_dep, (h0, w0), (h, w) = load_image_rgb_ir_dep(self, index)

            # Letterbox
            shape = self.batch_shapes_rgb[self.batch_rgb[index]] if self.rect else self.img_size  # final letterboxed shape
            img_rgb, ratio, pad = letterbox(img_rgb, shape, auto=False, scaleup=self.augment)
            img_ir, ratio, pad = letterbox(img_ir, shape, auto=False, scaleup=self.augment)
            img_dep, ratio, pad = letterbox(img_dep, shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            ...
            augment_hsv(img_rgb, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            augment_hsv(img_ir, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            augment_hsv(img_dep, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        ...
        img_ir = img_ir[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img_ir = np.ascontiguousarray(img_ir)
        img_dep = img_dep[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img_dep = np.ascontiguousarray(img_dep)

        img_all = np.concatenate((img_rgb, img_ir, img_dep), axis=0)
...
def load_image_rgb_ir_dep(self, index):
    # loads 1 image from dataset, returns img, original hw, resized hw

    img_rgb = self.imgs_rgb[index]
    img_ir = self.imgs_ir[index]
    img_dep = self.imgs_dep[index]
    # img_rgb = None
    # img_ir = None


    if (img_rgb is None) and (img_ir is None) and (img_dep is None):  # not cached

        path_rgb = self.img_files_rgb[index]
        path_ir = self.img_files_ir[index]
        path_dep = self.img_files_dep[index]
        ...
        img_rgb = cv2.imread(path_rgb)  # BGR
        img_ir = cv2.imread(path_ir)  # BGR
        img_dep = cv2.imread(path_dep)  # BGR

        assert img_rgb is not None, 'Image RGB Not Found ' + path_rgb
        assert img_ir is not None, 'Image IR Not Found ' + path_ir
        assert img_dep is not None, 'Image dep Not Found ' + path_dep
        ...
        if r != 1:  # if sizes are not equal
            ...
            img_ir = cv2.resize(img_ir, (int(w0 * r), int(h0 * r)),
                             interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
            img_dep = cv2.resize(img_dep, (int(w0 * r), int(h0 * r)),
                             interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
        return img_rgb, img_ir,img_dep, (h0, w0), img_rgb.shape[:2]  # img, hw_original, hw_resized
    else:
        return self.imgs_rgb[index], self.imgs_ir[index], self.imgs_dep[index], self.img_hw0_rgb[index], self.img_hw_rgb[index]  # img, hw_original, hw_resized
...
def load_mosaic_RGB_IR_DEP(self, index1, index2, index3):
    ...
    assert index_rgb == index_ir, 'INDEX RGB 不等于 INDEX IR'
    assert index_rgb == index_dep, 'INDEX RGB 不等于 INDEX dep'
    ...
    for i, index in enumerate(indices):
        ...
        # img, _, (h, w) = load_image(self, index)
        img_rgb, img_ir,img_dep, _, (h, w) = load_image_rgb_ir_dep(self, index)
        ...
        if i == 0:  # top left
            img4_rgb = np.full((s * 2, s * 2, img_rgb.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            img4_ir = np.full((s * 2, s * 2, img_ir.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            img4_dep = np.full((s * 2, s * 2, img_dep.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
        ...
        img4_rgb[y1a:y2a, x1a:x2a] = img_rgb[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        img4_ir[y1a:y2a, x1a:x2a] = img_ir[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        img4_dep[y1a:y2a, x1a:x2a] = img_dep[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        padw = x1a - x1b
        padh = y1a - y1b
        ...
        labels_rgb, segments_rgb = self.labels_rgb[index].copy(), self.segments_rgb[index].copy()
        labels_ir, segments_ir = self.labels_ir[index].copy(), self.segments_ir[index].copy()
        labels_dep, segments_dep = self.labels_dep[index].copy(), self.segments_dep[index].copy()
        if labels_rgb.size:
            labels_rgb[:, 1:] = xywhn2xyxy(labels_rgb[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            labels_ir[:, 1:] = xywhn2xyxy(labels_ir[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            labels_dep[:, 1:] = xywhn2xyxy(labels_dep[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            segments_rgb = [xyn2xy(x, w, h, padw, padh) for x in segments_rgb]
            segments_ir = [xyn2xy(x, w, h, padw, padh) for x in segments_ir]
            segments_dep = [xyn2xy(x, w, h, padw, padh) for x in segments_dep]
        labels4_rgb.append(labels_rgb)
        segments4_rgb.extend(segments_rgb)
        labels4_ir.append(labels_ir)
        segments4_ir.extend(segments_ir)
        labels4_dep.append(labels_dep)
        segments4_dep.extend(segments_dep)
    ...
    labels4_rgb = np.concatenate(labels4_rgb, 0)
    labels4_ir = np.concatenate(labels4_ir, 0)
    labels4_dep = np.concatenate(labels4_dep, 0)
    for x in (labels4_rgb[:, 1:], *segments4_rgb):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    for x in (labels4_ir[:, 1:], *segments4_ir):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    for x in (labels4_dep[:, 1:], *segments4_dep):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    # img4, labels4 = replicate(img4, labels4)  # replicate
    ...
    #                                    perspective=self.hyp['perspective'],
    #                                    border=self.mosaic_border)  # border to remove
    img4_rgb, img4_ir, img4_dep, labels4_rgb, labels4_ir = random_perspective_rgb_ir_dep(img4_rgb, img4_ir, img4_dep, labels4_rgb, labels4_ir,
                                                                           segments4_rgb, segments4_ir,
                                                    degrees=self.hyp['degrees'],
                                                    translate=self.hyp['translate'],
                                                    scale=self.hyp['scale'],
    ...
    return img4_rgb, labels4_rgb, img4_ir, labels4_ir, img4_dep, labels4_dep
...
def random_perspective_rgb_ir_dep(img_rgb, img_ir,img_dep, targets_rgb=(),targets_ir=(), segments_rgb=(), segments_ir=(),
                              degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            img_rgb = cv2.warpPerspective(img_rgb, M, dsize=(width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpPerspective(img_ir, M, dsize=(width, height), borderValue=(114, 114, 114))
            img_dep = cv2.warpPerspective(img_dep, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            # img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpAffine(img_ir, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_dep = cv2.warpAffine(img_dep, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
    ...
    return img_rgb, img_ir,img_dep, targets, targets
